...
<!--не сделано-->
<!--прощее вывести двумя массивами-->
<p><!-- разбил массивы на два-->
    <?php
    $arr10 = array(1, 100);
    $arr11 = array(-100, -1);
...
<!--решение где дан только один массив (опять хитрость) вывел на экран массив и просто посмотрел расположение элемента массива-->
<p>
    <?php
    $arr12 = range(-100, 100);
    echo "Минимальное значение =" . " " . $arr12[99] . ";";
    echo '<br>';
    echo "Максимальное значение =" . " " . $arr12[101] . ".";
    ?>
</p>
<p><!--Нашел решение в интернете разобрался, проще разденлить на два массива исключая значение 0, так как 0 в любом
случае самое минимальное число, что в отрицателньом, что в положителоносмм массиве-->
    <?php
    $array = [];
    for ($i = 0; $i < 100; $i++) {
        $array[$i] = mt_rand(-100, 100);
        echo $array[$i] . '<br>'; /* выводит последовательно 100 значение из которых потом будет произведена выборка*/
    }
    echo '<br>';
    $min = $max = $array[0];
...
<!--РЕШЕНИЕ найдено в интернете, сдела по подобию!!!!-->
<p>
    <?php
    $array22 = [];
    for ($i = 1; $i < 6; $i++) {
        for ($j = 1; $j < 7; $j++) {
            $array22[$j][$i] = mt_rand(1, 10);
            echo $array22[$j][$i] . ' ';
        }
        echo '<br>';
    }
